...
- [ ] 1.3 提供按 nodeType 查 NodeSpec 的查找入口
- [ ] 1.4 加载时校验:14 种类型全部存在,且每种 id_column / table 非空
...
- [ ] 2.3 编码 forward 平凡复合键边(2 条,match 含两列):
      | id                  | from  | to         | match (src→dst)                                |
...
- [ ] 2.4 编码 forward 语义边(2 条,列异名 + 显式假设标注):
...
- [ ] 2.5 编码 forward guard 边(1 条):
      | id            | from   | to   | match (src→dst)      | guard                                 |
      | bridge.beans  | bridge | bean | before_beans→bean_id | 按逗号拆分 before_beans 后逐项匹配 bean_id |
- [ ] 2.6 校验每条规则的 from / to 都能在 NodeSpec 中找到,否则加载报错
- [ ] 2.7 校验规则 id 全局唯一
- [ ] 2.8 校验:被标记为 guard 类别的边必须带 guard 声明,否则加载报错
- [ ] 2.9 提供「按 from 节点类型筛选 forward 边」的查找入口(含平凡/语义/guard 边)

## 3. forward 查询生成(build_expand_query)

- [ ] 3.1 实现 build_expand_query(rule, node, limit),仅生成 forward join
- [ ] 3.2 平凡边/语义边:对 rule.match 每一项生成「目标列 = 起点对应源列值」等值
      条件,多项以 AND 连接
- [ ] 3.3 guard 边:按 guard 声明先将起点源列值拆分为列表,再生成「目标列 IN
      (绑定参数列表)」条件
- [ ] 3.4 guard 边单值退化:源值仅含一项时生成含单个参数的 IN 条件
- [ ] 3.5 起点节点各取值(含 guard 拆分后的每个元素)以绑定参数传入,不做字符串拼接
- [ ] 3.6 生成的 SQL 含 LIMIT(由 limit 参数控制的安全上限)
- [ ] 3.7 SELECT 列表覆盖目标 NodeSpec 的 id_column + title + subtitle 字段

## 4. 测试

- [ ] 4.1 NodeSpec:任取一节点(如 flow)id_column 为 "id",title/subtitle 正确
- [ ] 4.2 NodeSpec:14 种类型枚举齐全
- [ ] 4.3 NodeSpec:按 nodeType 查找命中,未知类型返回空/报错
- [ ] 4.4 NodeSpec:缺失 id_column / table 时加载报错
- [ ] 4.5 EDGE_RULES:存在 service_entry→bean→java_class→java_method 的可达链
- [ ] 4.6 EDGE_RULES:存在 flow→state→activity→transition 的可达链
- [ ] 4.7 EDGE_RULES:logic→logic_step、logic→bridge 存在,且经 bridge→bean 接入
      bean 子树
- [ ] 4.8 EDGE_RULES:语义边 logic→activity / logic→flow_task 的 match 为
      chain_id→logic,且带假设标注
- [ ] 4.9 EDGE_RULES:guard 边 bridge→bean 的 match 为 before_beans→bean_id,
      且带拆分 guard 声明
- [ ] 4.10 EDGE_RULES:from/to 引用不存在的节点类型时加载报错
- [ ] 4.11 EDGE_RULES:重复规则 id 时加载报错
- [ ] 4.12 EDGE_RULES:guard 类别边缺 guard 声明时加载报错
- [ ] 4.13 build_expand_query:单列异名边(bean.java_class)SQL 快照正确,
      where 为 full_qualified_name = ?(绑定 bean_class 值)
- [ ] 4.14 build_expand_query:复合键边(state.activities)where 同时约束
      flow_id 与 state_name 两列
- [ ] 4.15 build_expand_query:语义边(logic.activities)where 为 logic = ?
      (绑定 chain_id 值)
- [ ] 4.16 build_expand_query:guard 边(bridge.beans)多值时 where 为
      bean_id IN (?, ?, ...),元素逐个绑定
- [ ] 4.17 build_expand_query:guard 边单值退化为含单参数的 IN
- [ ] 4.18 build_expand_query:含 LIMIT
- [ ] 4.19 build_expand_query:起点列值(含 guard 拆分元素)含 SQL 特殊字符时
      以参数绑定传入,查询语义不变