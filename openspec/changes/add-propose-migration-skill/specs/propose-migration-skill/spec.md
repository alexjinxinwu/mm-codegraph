...
### Requirement: 文件输出契约
skill MUST 在成功收到 MCP 返回后，向 `codeBaseRoot` 根目录写出一个纯文本文件，文件名为 `<id>-plan.txt`（`<id>` 即用户传入的原始 commandId 或 flowId，未做大小写/分隔符转换）。文件 MUST 为 UTF-8 编码、**只包含纯粹的文件列表**——每行一条受影响的源文件路径，**不写任何元信息头**（无 `# schema:` / `# status:` / `# reason:` 行）。
...
- **THEN** 文件正文为 17 行受影响源文件相对路径（按 MCP 返回顺序即字典序），首行直接是第一个文件路径、无元信息行
...
MCP 调用失败 / 受影响文件集合为空 / codeBaseRoot 路径不存在 时，skill **不写盘**。调用方在响应中贴出 Wrote / Entry point / File count / Warnings 四项，让调用者一眼看出调用成败与文件数量。`# status:` / `# reason:` 等元信息走响应而非文件，避免污染纯文件列表。

#### Scenario: MCP 返回错误
- **WHEN** `search_service_impact` 抛异常或返回 `{"error": "<msg>"}`
- **THEN** **不**写 `<id>-plan.txt`
- **THEN** skill 在回复中 MUST 把异常前 200 字原样呈现给用户

#### Scenario: 影响集为空
- **WHEN** `search_service_impact` 返回的影响文件集合为 0 条
- **THEN** 创建空文件 `<id>-plan.txt`（0 字节），不写占位文字
- **THEN** 不视为失败，正常结束（响应中 `File count: 0`）
