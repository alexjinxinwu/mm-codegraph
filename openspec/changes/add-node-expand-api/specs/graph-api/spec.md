...
- **WHEN** 提交 { schema: "S", node: { type: "flows", id: 10 } } 且不带 edgeIds
- **THEN** 返回该 flow 沿所有 from 为 flows 的 forward 边的邻居
- **AND** edges 中每条含 ruleId、from、to、label

#### Scenario: 仅展开指定边

- **WHEN** 提交 edgeIds: ["flow.states"]
- **THEN** 仅返回 flow.states 边的邻居,其余 from 为 flows 的边不展开
