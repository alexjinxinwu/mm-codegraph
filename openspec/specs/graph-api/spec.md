...
在 `codegraph-server` 暴露 HTTP 图探索 API，供前端逐层展开 ER 图邻居。核心端点为 **POST `/api/v1/expand`**（router prefix `/api/v1` + route `/expand`）：输入 `{ schema, node: { type, id }, edgeIds? }`，返回 `{ nodes, edges }`。

编排层调用 graph-core（`expand_neighbors` → 水合 → `build_expand_query` → `QueryEngine.q()`），不手写 expand SQL。

实现位置：`codegraph_core/graph/expand_service.py`、`codegraph_server/routes.py`、`codegraph_server/schemas_expand.py`。
