...
在 `codegraph_web/` 提供**图画布**：接收来自 browser-ui 外壳的种子节点，用 React Flow 渲染初始图，并通过 **POST `/api/v1/expand`** 支持点击节点增量展开邻居。维护去重后的节点/边集、dagre 增量布局，以及每节点 loading/error/retry 态。

实现位置：`codegraph_web/src/graph/`、`GraphCanvas.tsx`、`CodegraphNode.tsx`；依赖 browser-ui 提供的 `schema` 与 seed。
